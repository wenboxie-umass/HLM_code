...
int main(int argc, char** argv)
{
	...
	ofstream myfile;
	myfile.open("HL_KMP.txt", ios_base::trunc);
	...
	Profile *X_array = new Profile[N_thread];
	Profile *Y_array = new Profile[N_thread];
	Profile *X_times_Y_array = new Profile[N_thread];
	
	for(int i = 0 ; i < N_thread ; i++) {
		X_array[i].values.push_back(1);
		Y_array[i].values.push_back(1);
		X_times_Y_array[i].values.push_back(X_array[i].values.back() * Y_array[i].values.back());
	}
	...
	#pragma omp parallel num_threads(N_thread)
	{
		...
		big_step_distribute(clock_time_in_step,time_array,N+1,small_tau,ratio,out_n);
			...
			for(int in_n = 0; in_n < ratio; in_n++)
			{
				...
				{
					update(clock_time_in_step, in_n, N, small_tau, ratio, out_n, time_array, energy_array, u, r, count, X_array[rank], Y_array[rank], X_times_Y_array[rank], int(N/2));
				}
			}
		...
		info[rank].rank = rank;
		info[rank].runnging_time = delta;//1000000*delta/double(count);
	...
	for(int i = 0 ; i < N_thread ; i++) {
		...
		double cov = X_times_Y_array[i].summation/(Step*big_tau) - (X_array[i].summation/(Step*big_tau) * Y_array[i].summation/(Step*big_tau));
		cout<<"cov(X,Y) = E[XY] - E[X]E[Y] = "<<cov<<endl;
		double varX = variance(X_array[i].values);
		double varY = variance(Y_array[i].values);
	...
	for(int i = 0 ; i < N_thread ; i ++) {
		myfile<<"Thread "<<info[i].rank<<endl;
		myfile<<"total Cup time: "<<info[i].runnging_time;
		myfile<<"seconds per million event: "<<1000000*info[i].runnging_time/double(info[i].count)<<endl;
		myfile<<"=================================================="<<endl;
	}
